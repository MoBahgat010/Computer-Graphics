...
bool our::ShaderProgram::attach(const std::string &filename, GLenum type) const {
    ...
    glShaderSource(loadedSHader, 1, &sourceCStr, nullptr);
    glCompileShader(loadedSHader);

    std::string error = checkForShaderCompilationErrors(loadedSHader);
    if (!error.empty()) {
        std::cout << "Compilation Error in shader " << filename << ":\n" << error << std::endl;
        return false;
    }

    glAttachShader(program, loadedSHader);
