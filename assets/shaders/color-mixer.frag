...
void main(){
    float red_channel = dot(vec4(fs_in.color.rgb, 1.0), red);
    float green_channel = dot(vec4(fs_in.color.rgb, 1.0), green);
    float blue_channel = dot(vec4(fs_in.color.rgb, 1.0), blue);
